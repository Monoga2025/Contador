        ...
        datasets: [
          {
            ...
            data: daily.ingresos,
            borderColor: "#22e8c2",
            backgroundColor: "rgba(34,232,194,0.2)",
            tension: 0.3,
          },
          {
            label: "Gastos",
...
(function setupVoiceIA() {
  ...
  function addItemsToTable(items) {
    var tbody = document.querySelector("#quick-form .quick-table tbody");
    var firstRow = tbody && tbody.querySelector(".quick-row");
    if (!firstRow) return;
    var today = new Date().toISOString().slice(0, 10);
    items.forEach(function (it) {
      var tr = document.createElement("tr");
      tr.className = "quick-row";
      tr.innerHTML =
        '<td class="cell-incluir"><input type="checkbox" name="include_row" value="1" checked class="include-row-cb" aria-label="Incluir" /></td>' +
        '<td><input type="text" name="description" class="description-input-multi" value="' + (it.description || "").replace(/"/g, "&quot;") + '" /></td>' +
        '<td><input type="number" step="0.01" name="amount" value="' + (it.amount || 0) + '" /></td>' +
        '<td><input type="date" name="date_str" class="date-input-multi" value="' + today + '" aria-label="Fecha (calendario)" /></td>' +
        '<td><select name="type"><option value="ingreso">Ingreso</option><option value="gasto" selected>Gasto</option></select></td>' +
        '<td class="cell-category-auto"><span class="helper">La IA asigna al guardar</span><input type="hidden" name="category" value="" /></td>';
