(function () {
  ...
  // FIX: Mensajes Analytics claros — el usuario sabe cómo obtener datos
  function renderMonthly(data) {
    if (!data || !data.labels || data.labels.length === 0) {
      var wrap = document.getElementById("chart-monthly");
      if (wrap && wrap.parentElement) wrap.parentElement.innerHTML = "<p class='helper'>No hay datos de meses anteriores. Genera un cierre de mes (snapshot) desde Inicio o Config para ver la evolución aquí.</p>";
      return;
    }
  ...
  function renderProjection(data) {
    ...
    if (!data || data.proyeccion_fin_mes == null) {
      box.innerHTML = "<p class='helper'>No hay datos suficientes este mes. Añade movimientos en Inicio para ver la proyección.</p>";
      return;
    }
    box.innerHTML =
