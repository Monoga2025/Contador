...
# FIX: Import único de finance_engine — no importar dentro de cada función
...
class SavingsBalance(SQLModel, table=True):
    """Un solo registro: ahorro actual del usuario (cuánto tiene guardado). Se usa el primer registro; no crear más de uno."""
    __table_args__ = {"extend_existing": True}
    id: int | None = Field(default=None, primary_key=True)
    balance: float = 0.0
...
# FIX: Helper único para "solo movimientos reales" — evita duplicar la lógica de fijo mensual
def _es_movimiento_fijo(tx: Transaction) -> bool:
    return bool(tx.description and "fijo mensual" in (tx.description or ""))


def _es_movimiento_real(tx: Transaction) -> bool:
    return not _es_movimiento_fijo(tx)
...
def migrate_add_missing_columns() -> None:
    # FIX: Migraciones con log — no silenciar errores distintos a "columna ya existe"
    ...
    try:
        ...
        if "duplicate column" not in str(e).lower() and "already exists" not in str(e).lower():
            log.warning("migrate fixeditem.analytic_category: %s", e)
    try:
        with engine.connect() as conn:
        ...
        if "duplicate column" not in str(e).lower() and "already exists" not in str(e).lower():
            log.warning("migrate transaction.concept: %s", e)
...
def seed_fixed_items(session: Session) -> None:
    # FIX: Seed vacío — no inyectar datos de una persona concreta; usuario configura en /config
...
def refresh_current_month_from_fixed(session: Session) -> None:
    ...
    for tx in month_txs:
        ...
        if managed_by_fixed or _es_movimiento_fijo(tx):
            session.delete(tx)
    session.commit()
...
def _get_savings_balance(session: Session) -> float:
    """Singleton: devuelve el balance del primer (y único) registro SavingsBalance."""
...
def _pick_category_from_list(description: str, amount: float) -> str:
    ...
    if not model:
        return "Otros"
    # FIX: Clasificador con few-shot — ejemplos para que la IA responda solo una categoría de la lista
    list_str = ", ".join(f'"{c}"' for c in CATEGORIAS_ANALITICAS)
    prompt = f"""Clasificador de gastos/ingresos. Responde SOLO con una de estas categorías: {list_str}.

Ejemplos:
- "Arriendo 500000" -> Vivienda
- "Supermercado 80000" -> Alimentación
- "Gasolina 50000" -> Transporte
- "Netflix 20000" -> Servicios

Descripción: "{description}". Monto: {amount} COP.
Responde únicamente la categoría (una palabra o frase de la lista), sin explicación."""
...
def get_gemini_model():
    # FIX: Degradación graceful sin GEMINI_API_KEY — no romper la app; la IA devuelve mensaje claro
    api_key = os.getenv("AIzaSyBtj0vMIyz82FF2BT9GgGIB38sxkMmxEbg")
...
# FIX: Lifespan en lugar de on_event("startup") deprecado
...
app = FastAPI(title="Asesor Financiero IA", lifespan=lifespan)
...
def read_root(request: Request, session: Session = Depends(get_session)):
    ...
    today = _date.today()
    transactions_expected = [t for t in all_transactions if _es_movimiento_fijo(t) and t.date <= today]
    transactions_real = [t for t in all_transactions if _es_movimiento_real(t)]
    ...
    # FIX: Paginación implícita — solo últimos 60 días en "Todos los movimientos" para no sobrecargar la página
    from collections import defaultdict
    from datetime import timedelta
    cutoff = today - timedelta(days=60)
    transactions_para_vista = [t for t in transactions_real if t.date >= cutoff]
    por_dia = defaultdict(list)
    for t in transactions_para_vista:
...
def get_suggestions_concepts(
    ...
    real = [t for t in txs if _es_movimiento_real(t)]
...
async def get_advice(request: Request, session: Session = Depends(get_session)):
    ...
    # FIX: Pareja/planes desde perfil — solo inyectar si el usuario menciona pareja/novia en contexto
    profile = session.exec(select(UserProfile).limit(1)).first()
    ctx_personal = (profile.contexto_personal or "").lower() if profile else ""
    incluir_pareja = "pareja" in ctx_personal or "novia" in ctx_personal
    planes_pareja = (
        ["Acercarnos más a Dios", "Hacer un deporte juntos", "Conocer un pueblo cada 20 días", "Ir a piscina", "Acciones para mejorar en pareja"]
        if incluir_pareja else []
    )
    historial_chat = ""
    if history:
...
Responde en ESPAÑOL, MARKDOWN. Incluye: 1) Resumen de su situación. 2) Análisis con números. 3) Presupuesto recomendado. 4) Oportunidades de ahorro. 5) Plan de acción. Termina con 📌 Acción inmediata: una cosa concreta."""
    ...
    model = get_gemini_model()
    if not model:
        return {"advice": "**Asesor no disponible.** Configura `GEMINI_API_KEY` en el archivo `.env` para usar el asesor IA. Mientras tanto, revisa tus movimientos en Config y Objetivos.", "disponible": ctx["disponible"]}
...
def _build_daily_comparison(session: Session):
    ...
    expected = [t for t in transactions if _es_movimiento_fijo(t)]
    real = [t for t in transactions if _es_movimiento_real(t)]
...
def get_advice_hints(session: Session = Depends(get_session)):
    ...
    transactions_expected = [t for t in transactions if _es_movimiento_fijo(t) and t.date <= _date.today()]
    transactions_real = [t for t in transactions if _es_movimiento_real(t)]
    real_ingresos = sum(t.amount for t in transactions_real if t.type == "ingreso")
    real_gastos = sum(t.amount for t in transactions_real if t.type == "gasto")
    real_disponible = real_ingresos - real_gastos
...
def get_daily_briefing(session: Session = Depends(get_session)):
    ...
    fijos_hoy = [t for t in txs_hoy if _es_movimiento_fijo(t)]
...
Responde SOLO los 3 bullets, sin título ni introducción. Máximo 2 líneas por bullet."""
    ...
    try:
        model = get_gemini_model()
        if not model:
            return {"bullets": ["Configura GEMINI_API_KEY en .env para el asesor.", "Revisa tus movimientos en Config.", "Cada ahorro cuenta."], "score": ctx["score"], "runway": ctx["runway"]}
        r = model.generate_content(prompt)
        text = (r.text or "").strip() if hasattr(r, "text") else ""
...
Simula el impacto financiero: cómo afecta su disponible, ahorro, objetivos y runway. Da números concretos si puede estimar (ej. "Si compras X, tu disponible bajaría a Y"). Responde en español, MARKDOWN, máximo 15 líneas."""
    try:
        model = get_gemini_model()
        if not model:
            raise HTTPException(status_code=503, detail="Asesor no disponible. Configura GEMINI_API_KEY en .env")
        r = model.generate_content(prompt)
...
Máximo 20 líneas."""
    try:
        model = get_gemini_model()
        if not model:
            return {"advice": "Asesor no disponible. Configura GEMINI_API_KEY en el archivo .env para usar el análisis de cierre."}
        r = model.generate_content(prompt)
        texto = (r.text or "").strip() if hasattr(r, "text") else str(r)
        return {"advice": texto}
    except Exception:
...
"""
    try:
        model = get_gemini_model()
        if not model:
            return {"plan": "**Asesor no disponible.** Configura `GEMINI_API_KEY` en el archivo `.env` para generar el plan del mes. Mientras tanto, revisa Config (ingresos y gastos fijos) y Objetivos."}
        r = model.generate_content(SYSTEM_PROMPT_CFO + "\n\n" + prompt)
        text = (r.text or "").strip() if hasattr(r, "text") else ""
        return {"plan": text or "No se pudo generar el plan. Revisa tu perfil y movimientos."}
    except Exception:
        return {"plan": "No se pudo conectar con el asesor. Comprueba GEMINI_API_KEY en .env o intenta más tarde."}
...
"""
    try:
        model = get_gemini_model()
        if not model:
            return {"ideas": "Asesor no disponible. Configura GEMINI_API_KEY en .env para recibir ideas de ingreso. Completa tu perfil (ocupación, habilidades) para mejores sugerencias."}
        r = model.generate_content(SYSTEM_PROMPT_CFO + "\n\n" + prompt)
        text = (r.text or "").strip() if hasattr(r, "text") else ""
        return {"ideas": text or "No se pudieron generar ideas. Completa tu perfil (ocupación, habilidades)."}
    except Exception:
        return {"ideas": "No se pudo conectar con el asesor. Comprueba GEMINI_API_KEY en .env o intenta más tarde."}
...
4) Un truco práctico (ej. "Aparta el mismo día que cobras"). Máximo 12 líneas."""
    try:
        model = get_gemini_model()
        if not model:
            return {"advice": "Asesor no disponible. Configura GEMINI_API_KEY en .env para la estrategia por objetivo.", "proyeccion": proy}
        r = model.generate_content(prompt)
        texto = (r.text or "").strip() if hasattr(r, "text") else str(r)
        return {"advice": texto, "proyeccion": proy}
    except Exception:
        return {"advice": "No se pudo generar la estrategia. Comprueba GEMINI_API_KEY en .env.", "proyeccion": proy}
...
async def parse_expenses_from_text(request: Request):
    ...
    model = get_gemini_model()
    if not model:
        raise HTTPException(status_code=503, detail="Asesor no disponible. Configura GEMINI_API_KEY en .env para extraer gastos del texto.")
    ...
    try:
        ...
        if raw.startswith("```"):
            ...
            if raw.startswith("json"):
                raw = raw[4:]
        raw = raw.strip()
        # FIX: Parseo JSON robusto — extraer array aunque la IA envuelva en texto
        import re as _re
        ...
        raise HTTPException(status_code=422, detail="No se pudieron extraer ítems del texto. Prueba con: pan 5000, café 3000.")
...
async def parse_expenses_from_image(
    ...
    model = get_gemini_model()
    if not model:
        raise HTTPException(status_code=503, detail="Asesor no disponible. Configura GEMINI_API_KEY en .env para leer facturas.")
    # Determinar mime
...
Sin explicaciones, solo el JSON array."""
    try:
        ...
        raise HTTPException(status_code=422, detail="No se pudieron leer ítems de la factura. Asegúrate de que la imagen sea legible y tenga montos en COP.")
...
async def remap_category(
    ...
    # FIX: remap_category — documentación alineada: se reasigna por categoría analítica (category), no por concepto
    """Reasigna todas las transacciones cuya categoría analítica es 'from_label' a la categoría 'to_label'."""
    body = await request.json()
...
"""
    ...
    model = get_gemini_model()
    if not model:
        return {"advice": "**Asesor no disponible.** Configura GEMINI_API_KEY en .env para consejos sobre objetivos."}
    response = model.generate_content(prompt)
    texto = response.text if hasattr(response, "text") else str(response)
    return {"advice": texto}
...
def _transacciones_reales_mes(session: Session, year: int, month: int):
    ...
    txs = list(session.exec(stmt).all())
    return [t for t in txs if _es_movimiento_real(t)]
